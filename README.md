...
浏览器打开 [http://127.0.0.1:8765/](http://127.0.0.1:8765/)。页面提供下单表单、提交前确认、查件表单、轨迹时间线和结构化结果。状态栏只显示运行环境与凭据是否已配置，不展示顾客编码或校验码。页面不会使用 Cookie 或浏览器本地存储；刷新即清空填写内容。
...
缺少凭据时可先检查页面和输入校验；实际沙盒联调仍需在启动服务的进程环境中设置 `SF_PARTNER_ID`、`SF_CHECK_WORD` 及顺丰应用接口权限。修改环境变量后须重启网页服务。
