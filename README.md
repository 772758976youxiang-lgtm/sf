...
浏览器打开 [http://127.0.0.1:8765/](http://127.0.0.1:8765/)。先在页面顶部输入顾客编码、校验码，并选择与顺丰测试平台一致的签名方式。网页默认使用简易 MD5；若平台选择标准 MD5，请在网页切换。保存后即可用于当前网页的下单与查件请求，无需重启服务。保存只表示已填写，真正的签名和权限校验发生在调用顺丰接口时。页面还提供清除网页配置的按钮。凭据只保存在当前 Python 进程内，重启后失效；若未在网页配置，仍会使用启动进程的环境变量。状态栏只显示运行环境、签名方式与凭据是否已填写，不展示顾客编码或校验码。页面不会使用 Cookie 或浏览器本地存储；刷新即清空填写内容。
...
网页服务只监听 `127.0.0.1`，仅接受同源的 JSON 请求。**网页下单始终限定沙盒**，即使设置了 `SF_ALLOW_PRODUCTION_ORDERS=true` 也不会从网页创建生产订单。查询使用当前 `SF_ENV` 所选环境。关闭运行它的终端或按 `Ctrl+C` 即停止网页服务。

缺少凭据时可先检查页面和输入校验；实际沙盒联调需在网页填写凭据，或在启动服务的进程环境中设置 `SF_PARTNER_ID`、`SF_CHECK_WORD`，并确保顺丰应用已获得接口权限。修改环境变量后须重启网页服务。
